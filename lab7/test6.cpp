...
void producer() {
  ...
  while (quit.load() == false) {
    int job = rand() % 100;
    {
        ...
        //while로 하는 이유: 깨어났는데 잡이 없을 경우 다시 잠들어야 하기 때문
        while(que[0]!=NO_JOB){ //큐에 job이 있으면
            queFillable.wait(ul); //큐에 남은 자리가 있는지 확인하고 기다린다. 
        }
        que[0]=job;
...
void consumer() {
  ...
  while (quit.load() == false) {
   ...
   {
    unique_lock<mutex> ul(queMutex);
    while(que[0]==NO_JOB){ //큐가 비어있지 않으면 반복해서 비어있는지 물어보면서 기다린다.
        queFilled.wait(ul);
    }
    job = que[0];
    que[0] = NO_JOB;
